...
export default function (self) {
  ...
  self.setDom = function () {
    if (self.isChildGrid) {
      ...
      // Create animated cell selection overlay
      if (!self.parentIsCanvas) {
        var overlayParent = self.parentDOMNode && self.parentDOMNode.nodeType === 1
          ? self.parentDOMNode
          : (self.parentDOMNode && self.parentDOMNode.appendChild ? self.parentDOMNode : null);
        if (overlayParent) {
          self.selectionOverlay = document.createElement('div');
          self.selectionOverlay.style.cssText =
            'position:absolute;pointer-events:none;box-sizing:border-box;z-index:1;display:none;';
          if (overlayParent.style &&
              (!overlayParent.style.position || overlayParent.style.position === 'static')) {
            overlayParent.style.position = 'relative';
          }
          overlayParent.appendChild(self.selectionOverlay);
        }
      }
  ...
  self.updateSelectionOverlay = function (cell) {
    ...
    if (!el || !self.attributes.animatedCellSelection) {
      return;
    }
    if (!cell) {
    ...
    // Position relative to canvas element's top-left
