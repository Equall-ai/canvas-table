...
export default function (self) {
  ...
  self.updateSelectionOverlay = function (cell) {
    if (!self.attributes.animatedCellSelection) {
      return;
    }
    // Lazily create overlay and wrapper on first use
    var el = self.selectionOverlay;
    if (!el || !el.parentNode) {
      if (!self.canvas || !self.canvas.parentNode) {
        return;
      }
      el = document.createElement('div');
      el.className = 'cdg-selection-overlay';
      el.style.cssText =
        'position:absolute;top:0;left:0;pointer-events:none;box-sizing:border-box;z-index:1;display:none;';
      // Ensure canvas is inside a positioned wrapper
      var parent = self.canvas.parentNode;
      var wrapper = parent.querySelector('.cdg-selection-wrapper');
      if (!wrapper) {
        wrapper = document.createElement('div');
        wrapper.className = 'cdg-selection-wrapper';
        wrapper.style.cssText = 'position:relative;width:100%;height:100%;';
        parent.insertBefore(wrapper, self.canvas);
        wrapper.appendChild(self.canvas);
      }
      wrapper.appendChild(el);
      self.selectionOverlay = el;
    }
    if (!cell) {
      el.style.display = 'none';
      self._overlayActiveRow = undefined;
      self._overlayActiveCol = undefined;
      return;
    }
    var duration = self.attributes.animatedCellSelectionDuration + 'ms';
    var color = self.style.activeCellOverlayBorderColor;
    var width = self.style.activeCellOverlayBorderWidth;
    var ratio = self.scale || 1;
    var left = cell.x / ratio;
    var top = cell.y / ratio;
    var w = cell.width / ratio;
    var h = cell.height / ratio;
    // Determine if the active cell actually changed (navigation/click) vs
    // just repositioned (scroll). Only animate on cell change.
    var activeRow = self.activeCell ? self.activeCell.rowIndex : undefined;
    var activeCol = self.activeCell ? self.activeCell.columnIndex : undefined;
    var cellChanged = activeRow !== self._overlayActiveRow || activeCol !== self._overlayActiveCol;
    self._overlayActiveRow = activeRow;
    self._overlayActiveCol = activeCol;
    var now = performance.now();
    var animating = self._overlayTransitionEnd && now < self._overlayTransitionEnd;
    if (el.style.display === 'none') {
      ...
      el.style.transition =
        ...
        'width ' + duration + ' ease-out, ' +
        'height ' + duration + ' ease-out';
      self._overlayTransitionEnd = now + self.attributes.animatedCellSelectionDuration;
    } else if (!animating) {
      // Only snap if no transition is in progress — avoids cancelling
      // an active animation when draw() fires multiple times per frame.
      el.style.transition = 'none';
    }
    // Clip the overlay so it doesn't cover frozen columns or frozen rows.
    // The overlay is positioned absolutely in the wrapper, so we clip using
    // the wrapper's coordinate space via a polygon that excludes the frozen area.
    var isFrozenCell =
      (self.activeCell.columnIndex < self.frozenColumn) ||
      (self.activeCell.rowIndex < self.frozenRow);
    if (!isFrozenCell && self.frozenColumn > 0) {
      var frozenPx = (self.lastFrozenColumnPixel || 0) / ratio;
      // Clamp: if the overlay's right edge is past the frozen boundary, clip the left side
      if (left < frozenPx) {
        var clipAmount = frozenPx - left;
        el.style.clipPath = 'inset(0 0 0 ' + clipAmount + 'px)';
      } else {
        el.style.clipPath = '';
      }
    } else {
    ...
    el.style.borderRadius = self.attributes.animatedCellSelectionBorderRadius + 'px';
  };
}
