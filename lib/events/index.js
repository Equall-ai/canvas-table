...
export default function (self) {
        ...
        if (
          ...
          if (
            ...
            if (cell.hovered && self.hovers.onFilterButton) {
              ...
              if (
                ctrl &&
                self.preCtrlDragSelections &&
                cell.rowIndex !== undefined
              ) {
                self.selections = cloneSelections(self.preCtrlDragSelections);
                const rect = normalizeSelection({
                  type: SelectionType.Cells,
                  startRow: dragBounds.top,
                  endRow: dragBounds.bottom,
                  startColumn: dragBounds.left,
                  endColumn: dragBounds.right,
                });
                const context = {
                  rows: self.viewData.length,
                  columns: self.getSchema().length,
                };
                if (self.dragAddToSelection) {
                  addIntoSelections(self.selections, rect, context);
                } else {
                  removeFromSelections(self.selections, rect, context);
                }
                self.dispatchSelectionChangedEvent();
              } else if (
                !self.dragAddToSelection &&
                cell.rowIndex !== undefined
              ) {
  ...
  self.mousedown = function (e, overridePos) {
    ...
    self.preCtrlDragSelections = ctrl
      ? cloneSelections(self.selections)
      : undefined;
    if (
      !ctrl &&
      !e.shiftKey &&
  ...
  self.mouseup = function (e) {
    ...
    self.preCtrlDragSelections = undefined;
    if (
  ...
  self.keydown = function (e) {
    ...
    if (e.shiftKey && isArrowKey) {
      // If no selection exists, create one from the active cell first
      if (self.selections.length === 0) {
        self.selectCell(
          {
            rowIndex: self.activeCell.rowIndex,
            viewColumnIndex: self.activeCell.columnIndex,
          },
          true,
        );
      }
    ...
    if (x !== self.activeCell.columnIndex || y !== self.activeCell.rowIndex) {
      ...
      } else if (!e.shiftKey && !ctrl && isArrowKey) {
        self.clearSelections();
      }
      self.draw(true);
    }
