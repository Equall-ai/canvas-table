...
export default function (self) {
  ...
  /**
   * Reconcile `self.activeCell` with the current `self.selections` after an
   * external mutation. If the active cell still falls inside the new list,
   * leave it alone; otherwise move it to the anchor of the first descriptor,
   * or off-grid (-1,-1) when the list is empty. Without this, the active-cell
   * border stays painted on the old cell forever.
   */
  const reconcileActiveCellWithSelections = () => {
    if (!self.activeCell) return;
    if (self.selections.length === 0) {
      self.activeCell.rowIndex = -1;
      self.activeCell.columnIndex = -1;
      return;
    }
    if (
      isCellSelected(
    ...
    const first = self.selections[0];
    self.activeCell.rowIndex = first.startRow ?? 0;
    self.activeCell.columnIndex = first.startColumn ?? 0;
  };

  /**
   * Replace the current selection list with the given array of selection descriptors.
   * Use this to programmatically set a non-contiguous selection in one call.
   * @param {SelectionDescriptor[]} list
   * @param {boolean} [suppressEvent]
   * @returns {boolean} whether the selection changed
   */
  self.setSelectionList = (list, suppressEvent) => {
    if (!Array.isArray(list)) {
      throw new TypeError(
        'selectionList must be an array of selection descriptors',
      );
    }
    const next = [];
    for (const desc of list) {
      if (!desc || typeof desc.type !== 'number') {
        throw new TypeError(
          'Each selection descriptor must have a numeric `type` field',
        );
      }
      next.push(normalizeSelection(Object.assign({}, desc)));
    }
    cleanupSelections(next);
    self.selections = next;
    reconcileActiveCellWithSelections();
  ...
  /**
   * Append a selection descriptor without clearing the existing selection list.
   * @param {SelectionDescriptor} descriptor
   * @param {boolean} [suppressEvent]
   * @returns {boolean} whether the selection changed
   */
  self.addSelection = (descriptor, suppressEvent) => {
    if (!descriptor || typeof descriptor.type !== 'number') {
      throw new TypeError(
        'selection descriptor must have a numeric `type` field',
      );
    }
    const context = {
      rows: self.viewData ? self.viewData.length : 0,
      columns: self.getSchema().length,
    };
    const changed = addIntoSelections(
      self.selections,
      normalizeSelection(Object.assign({}, descriptor)),
      context,
    );
    if (changed) {
      reconcileActiveCellWithSelections();
  ...
  /**
   * Remove the cells/rows/columns described by `descriptor` from the selection list.
   * @param {SelectionDescriptor} descriptor
   * @param {boolean} [suppressEvent]
   * @returns {boolean} whether the selection changed
   */
  self.removeSelection = (descriptor, suppressEvent) => {
    if (!descriptor || typeof descriptor.type !== 'number') {
      throw new TypeError(
        'selection descriptor must have a numeric `type` field',
      );
    ...
    const changed = removeFromSelections(
      self.selections,
      normalizeSelection(Object.assign({}, descriptor)),
      context,
    );
